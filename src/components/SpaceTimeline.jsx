...
export const ScrollTimeline = ({
  ...
  timelineColor = "linear-gradient(to bottom, #000000, #333333)",
}) => {
  ...
  const progressLineHeight = useTransform(smoothProgress, [1, 0], ["0%", "100%"]);



  // Create yOffset transformations for all events upfront
  const yOffsets = events.map(() =>
    useTransform(
      smoothProgress,
      [0, 1],
  ...
  const getCardClasses = (index) => {
    ...
    const variantClasses = {
      default: `${cardBackground} border ${cardBorderColor} shadow-sm`,
      elevated: `${cardBackground} border ${cardBorderColor} shadow-md`,
      outlined: `${cardBackground}/50 backdrop-blur border-2 ${cardBorderColor}`,
      filled: `${cardBackground} border ${cardBorderColor}`,
    };
    const effectClasses = {
      none: "",
      glow: "hover:shadow-[0_0_15px_rgba(0,0,0,0.5)]",
            ...
            <>
              ...
              >
                <motion.div
                  ...
                  style={{
                    background:
                      "radial-gradient(circle, rgba(0,0,0,0.8) 0%, rgba(51,51,51,0.5) 40%, rgba(0,0,0,0) 70%)",
                    boxShadow: `
                      0 0 15px 4px rgba(0,0,0,0.6),
                      0 0 25px 8px rgba(51,51,51,0.4),
              ...
              return (
                ...
                >
                  ...
                  >
                    <motion.div
                      className={cn(
                        "w-6 h-6 rounded-full border-4 bg-white flex items-center justify-center",
                        index <= activeIndex ? "border-black" : "border-gray-300 bg-white"
                      )}
                      animate={
                        index <= activeIndex
                          ? {
                  ...
                  <motion.div
                    ...
                    viewport={{ once: false, margin: "-100px" }}
                    style={parallaxIntensity > 0 ? { y: yOffsets[index] } : undefined}
                  >
                    <Card className={`${cardBackground} ${cardBorderColor}`}>
                        ...
                        {dateFormat === "badge" ? (
                          <div className="flex items-center mb-2">
                            ...
                            <span
                              className={cn(
                                "text-sm font-bold",
                                event.color ? `text-${event.color}` : "text-black"
                              )}
                            >
                              {event.year}
                            </span>
